...
const DragGroupV2 = ({
    ...
    const onDragOverItem = event => {
        if (maxHeight != '') scrollBarFun(event);
    ...
    const scrollBarFun = event => {
        ...
        if (num > refDom.getBoundingClientRect().top - 100) {
    ...
    return (
        <div
            id="dragV3"
            ref={domRef}
            style={maxHeight != '' ? { maxHeight: maxHeight, overflowX: 'auto' } : {}}
